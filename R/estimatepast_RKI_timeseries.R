...
#' \emph{EstimationCumNumberIllPresent} employs \emph{EstimationCumNumberIllPast} to estimate the number of actually 
...
#' With \emph{EstimationCumNumberIllPresent} and \strong{prop_death} we can calculate an expected number of deaths
#' \emph{EstimationCumNumberDead}.
#'
      ...
      y %>%
        dplyr::mutate(
          EstimationCumNumberIllPast = dplyr::lead(.data[["CumNumberDead"]], mean_days_until_death - 1) / 
            prop_death,
          EstimationCumNumberIllPresent = dplyr::lag(.data[["EstimationCumNumberIllPast"]], mean_days_until_death - 1) * 
            (2^(mean_days_until_death/doubling_time)),
          EstimationCumNumberDead = dplyr::lag(.data[["EstimationCumNumberIllPresent"]], mean_days_until_death - 1) * 
            prop_death
        )
    }
  ) %>%
