...
rki <- get_RKI_timeseries(cache = T)
...
p <- cowplot::plot_grid(
  p1, 
  p2, 
  p4 + ggplot2::scale_y_continuous(labels = scales::comma), 
  p5, 
  nrow = 2, ncol = 2, rel_heights = c(1,0.8), align = "v", axis = 'l'
)
...
p_vac <- vac_ts %>%
  dplyr::mutate(
    CumNumberVaccinated = cumsum(NewNumberVaccinated)
  ) %>%
  ggplot() +
  geom_line(aes(x = Date, y = CumNumberVaccinated)) +
  scale_y_continuous(labels = scales::comma) +
  ggtitle(paste0(lubridate::today(), ": Total number of (first) vaccination doses administered")) +
  theme_minimal() +
  theme(axis.title.x = element_blank())

ggplot2::ggsave("man/figures/README_plot_vac.png", device = "png", p_vac, width = 10, height = 3, scale = 0.8)
```
